<template>
  <div class="edit_flow">
    <div class="editFlow_header">
      ...
      <el-button type="success" @click="finish">完成任务</el-button>
      <el-button type="danger" @click="backToPreviousTask">退回</el-button>

      <el-dropdown split-button type="primary">
        流程操作
        <el-dropdown-menu slot="dropdown">
          <el-dropdown-item @click.native="addFlowTask('1')">加签</el-dropdown-item>
          <el-dropdown-item @click.native="transferFlowTask('2')">转签</el-dropdown-item>
          <el-dropdown-item @click.native="circulateFlowTask('3')">传阅</el-dropdown-item>
        </el-dropdown-menu>
      </el-dropdown>
    ...
    <div class="flow_success">
      ...
      >
        ...
        <div v-if="tableList.length > 0" style="margin-top:30px">
          <span>请选择下一个执行节点：</span>
          ...
          >
            ...
            <el-table-column :label="'执行人'" width="180px" align="center">
              <template slot-scope="scope">
                <span>{{scope.row.sponsorName | filterData}}</span>
              </template>
            </el-table-column>
          </el-table>
        </div>
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(val) {
      self.arr = [];
      val.forEach(element => {
        self.arr.push(element.id);
      });
    },
    finish() {
      GetNextNode({ input: localStorage.getItem("flowTaskLinkId") }).then(
        res => {
...
.flow_success .el-table td,
.flow_success .el-table th {
