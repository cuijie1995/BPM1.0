<template>
  <div>
    <el-form-item
      ...
      :class="{active: selectFormData.key === element.key, 'is_req': element.options.required}"
      :label="element.name"
      @click.native.stop="handleSelectForm(index)"
    >
      ...
      <div class="created-view-action" v-if="selectFormData.key === element.key">
        <i class="el-icon-document-copy" title="复制" @click.stop="handleCloneForm(index)"></i>
        <i class="el-icon-delete" title="删除" @click.stop="handleDeleteForm(index)"></i>
      </div>

      <div class="created-view-drag" v-if="selectFormData.key === element.key">
        <i title="拖动" class="el-icon-sort drag-form"></i>
      </div>
    </el-form-item>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handleCloneForm(index) {
      let cloneData = {
        ...this.data.dynamicList[index],
        options: { ...this.data.dynamicList[index].options },
        key: Date.parse(new Date()) + "_" + Math.ceil(Math.random() * 99999)
      };
      if (
        this.data.dynamicList[index].type === "radio" ||
        this.data.dynamicList[index].type === "checkbox" ||
        this.data.dynamicList[index].type === "select"
      ) {
        cloneData = {
          ...cloneData,
          options: {
      ...
      this.data.dynamicList.splice(index, 0, cloneData);
      this.$nextTick(() => {
        this.selectFormData = this.data.dynamicList[index + 1];
      });
    }
