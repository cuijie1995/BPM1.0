...
export default {
  ...
  methods: {
    ...
    getCheckedNodes() {
      ...
      self.starter.department = data.map(item => item.id);
      var department = [];
    ...
    handlePostChange(val) {
      ...
      self.starter.post = self.$refs.postTable.store.states.selection.map(
        item => item.id
      );
      var post = [];
    ...
    handleUserChange(val) {
      self.starter.user = self.$refs.userTable.store.states.selection.map(
        item => item.id
      );
      var user = [];
    ...
    handleRoleChange(val) {
      self.starter.role = val.map(item => item.id);
      var role = [];
    ...
    confirm() {
      self.starterName =
        ...
        "," +
        self.departmentName;
      // console.log(self.starter);
      // console.log(self.starterName);
      self.$emit("checkSponor", self.starter, self.starterName);
    ...
    sonFun(data) {
      ...
      if (data.starter && data.starter != "") {
        // self.starter = JSON.parse(JSON.stringify(data.starter));
  ...
  watch: {
    flowData: {
      handler(val) {
        if (val.starter && val.starterName != "") {
          self.starterName = val.starterName;
        }
        if (val.starter && val.starter != "") {
          self.starter = JSON.parse(val.starter);
        }
      }
    },
    deep: true
