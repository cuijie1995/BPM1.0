<template>
  <div style="width: 100%; height: 100%; overflow: hidden; position: relative;" ref="flowContainer">
    ...
    <draggable
      ...
      :class="{canScale: container.scaleFlag, canDrag: container.dragFlag, canMultiple: rectangleMultiple.flag }"
    ...
    <div
      ...
      class="flow-container"
      style="cursor: pointer;"
      :class="{grid: true,canScale: container.scaleFlag, canDrag: container.dragFlag, canMultiple: rectangleMultiple.flag }"
    ...
    >
      <template v-for="node in flowData.flowNodes">
        <flow-node
          :key="node.id"
          v-if="node && node.id"
          :node="node"
...
export default {
  ...
  methods: {
    ...
    handleAddFormItem(evt) {
      ...
      this.mouse.position.left = flag
        ? evt.originalEvent.offsetX + evt.originalEvent.target.offsetLeft
        : evt.originalEvent.offsetX;
      this.mouse.position.top = flag
        ? evt.originalEvent.offsetY + evt.originalEvent.target.offsetTop
        : evt.originalEvent.offsetY;
      this.$nextTick(() => {
        const newIndex = evt.newIndex;
        const nodeName = this.flowData.flowNodes[newIndex].nodeName;
        const belongTo = this.flowData.flowNodes[newIndex].belongTo;

        this.$emit("findNodeConfig", belongTo, nodeName, node => {
          if (!node) {
    ...
    addNewNode(node, index) {
      ...
      newNode.id = ZFSN.getId();
      newNode.height = 50;
      if (newNode.nodeName === "开始节点" || newNode.nodeName === "工作节点") {
        newNode.left = x - 25;
        newNode.width = 50;
      } else {
        newNode.left = x - 60;
      ...
      newNode.top = y - 25;
      if (newNode.nodeName === "横向泳道") {
        newNode.height = 200;
        newNode.width = 600;
      } else if (newNode.nodeName === "纵向泳道") {
        newNode.height = 600;
        newNode.width = 200;
      }

      this.$set(this.flowData.flowNodes, index, { ...newNode });
    ...
    mousemoveHandler(e) {
      ...
      if (that.container.startPos.startMove) {
        ...
        const YM =
          that.container.endPos.y -
          (that.container.startPos.y - that.container.startPos.t);
        const XM =
          that.container.endPos.x -
          (that.container.startPos.x - that.container.startPos.l);
        const yFlag =
          Math.abs(that.container.endPos.y - that.container.startPos.y) >= 20;
        const xFlag =
          Math.abs(that.container.endPos.x - that.container.startPos.x) >= 20;
        that.container.pos = {
      ...
      if (that.container.draging) {
        let nTop =
          ...
          (that.mouse.position.top - that.mouse.tempPos.top);
        let nLeft =
          that.container.pos.left +
          (that.mouse.position.left - that.mouse.tempPos.left);
        if (nTop >= 0) nTop = 0;
        if (nLeft >= 0) nLeft = 0;
        that.container.pos = {
          top: nTop,
    ...
    paste() {
      ...
      that.clipboard.forEach(function(node, index) {
        const newNode = Object.assign({}, node);
        newNode.id = ZFSN.getId();
        const nodePos = that.computeNodePos(
          that.mouse.position.left + dis,
          that.mouse.position.top + dis
        );
        newNode.left = nodePos.left;
        newNode.top = nodePos.top;
        dis += 20;
        that.flowData.flowNodes.push(newNode);
      });
    },
    selectAll() {
      const that = this;
      ...
      console.log(that.currentSelectGroup);
    ...
    verticaLeft() {
      ...
      for (let i = 1; i < selectGroup.length; i++) {
        ...
        const f = flowNodes.filter(n => n.id === selectGroup[i].id)[0];
        f.tx = baseX;
        f.ty = baseY;
        that.plumb.animate(
          selectGroup[i].id,
    ...
    deleteNode() {
      const that = this;
      const flowNodes = that.flowData.flowNodes;
      const flowNodeMaps = that.flowData.flowNodeMaps;
      that.flowData.flowNodeMaps = flowNodeMaps.filter(
        link =>
          link.flowNodeParentId !== this.currentSelect.id &&
          link.flowNodeChildId !== this.currentSelect.id
      );
      that.plumb.deleteConnectionsForElement(this.currentSelect.id);
      const inx = flowNodes.findIndex(
        node => node.id === this.currentSelect.id
      );
      flowNodes.splice(inx, 1);
    ...
    handleConnect() {
      ...
      that.$nextTick(() => {
        const attrLines = [...that.flowData.flowNodeMaps];
        attrLines.forEach(function(link, index) {
          const conn = that.plumb.connect({
            source: link.flowNodeParentId, //连线源的标识，可以是id, element, 或者Endpoint
            target: link.flowNodeChildId, //连线目标的标识，可以是id, element, 或者Endpoint
            anchor: flowConfig.jsPlumbConfig.anchor.default,
            connector: [
              //设置链接线的形状  Bezier: 贝塞尔曲线 Flowchart: 具有90度转折点的流程线 StateMachine: 状态机 Straight: 直线
              link.linkType,
              {
                gap: 5,
                cornerRadius: 2,
    ...
    updateNodePos() {
      ...
      that.currentSelectGroup.forEach(function(node, index) {
        ...
        const f = flowNodes.filter(n => n.id === node.id)[0];
        f.left = l;
        f.top = t;
      });
    },
  ...
  watch: {
    select(val) {
      ...
      //   // 监听esc键退出全屏
      //   if (event.keyCode == 17) {
      //     // this.plumb.addToDragSelection(val.id);
      //     this.currentSelectGroup.push(val);
      //   }
      ...
      if (this.tempLinkId !== "") {
        document.getElementById(this.tempLinkId) &&
          document.getElementById(this.tempLinkId).classList &&
          document
            .getElementById(this.tempLinkId)
      ...
      if (this.currentSelect.type === "sl") {
        ...
        document
          .getElementById(this.currentSelect.id)
