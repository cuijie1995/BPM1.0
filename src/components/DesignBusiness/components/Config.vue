<template>
  <div v-if="show">
    <el-form label-position="top" :rules="rules">
      ...
      <el-form-item :label="'布剧'" v-if="Object.keys(data.options).indexOf('inline')>=0">
      ...
      <el-form-item :label="'多选'" v-if="data.type=='select' || data.type=='imgupload'">
        <el-switch size="small" v-model="data.options.multiple" @change="handleSelectMuliple"></el-switch>
      </el-form-item>
      <el-form-item :label="'可搜索'" v-if="data.type=='select'">
        <el-switch v-model="data.options.filterable"></el-switch>
      </el-form-item>
      <el-form-item label="'允许半边'" v-if="Object.keys(data.options).indexOf('allowHalf')>=0">
        ...
        <template>
          ...
          <template v-if="data.type=='radio' || (data.type=='select'&&!data.options.multiple)">
            ...
            >
              <draggable
                tag="ul"
                :dynamicList="data.options.options"
              ...
              >
                <li v-for="(item, index) in data.options.options" :key="index">
                  <el-radio :label="item.value" style="margin-right: 5px;">
                    <el-input
                      :style="{'width': data.options.showLabel? '80px': '180px' }"
                      size="mini"
                      v-model="item.value"
                    ></el-input>
                    <el-input
                  ...
                  <i class="drag-item" style="font-size: 16px;margin: 0 5px;cursor: move;">
                    <i class="el-icon-rank" title="拖动"></i>
                  </i>
                  <el-button
      ...
      <el-form-item v-if="type == 0" :label="'标识属性'">
        <el-input v-model="data.identification" size="small"></el-input>
      </el-form-item>

      <el-form-item
        :label="'默认值'"
        v-if="Object.keys(data.options).indexOf('defaultValue')>=0 && (data.type == 'textarea' || data.type == 'input' ||  data.type == 'text' || data.type=='rate' || data.type=='color' || data.type=='switch')"
      >
        <el-input
          size="small"
      ...
      <template v-if="data.type == 'grid'">
        ...
        <el-form-item :label="'列配置项'">
          ...
          <div style="margin-left: 22px;">
            <el-button size="small" type="text" @click="handleAddColumn">添加列</el-button>
          </div>
        </el-form-item>
        ...
        <el-form-item :label="'垂直排列方式'">
          <el-select size="small" v-model="data.options.align">
            <el-option value="top" :label="'顶对齐'"></el-option>
            <el-option value="middle" :label="'居中'"></el-option>
            <el-option value="bottom" :label="'底对齐'"></el-option>
          </el-select>
        </el-form-item>
      </template>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        identification: [
          { required: true, message: "标识属性为必填值", trigger: "blur" }
        ]
      }
    };
  },
