<template>
  <div class="created-form-content">
    ...
    >
      <template v-for="item in data.dynamicList">
        <template v-if="item.type == 'grid'">
          ...
          >
            <el-col v-for="(col, colIndex) in item.columns" :key="colIndex" :span="col.span">
              <template v-for="citem in col.dynamicList">
                <el-form-item
...
export default {
  ...
  created() {
    this.generateModle(this.data.dynamicList);
  },
  mounted() {},
  methods: {
    generateModle(arr) {
          ...
          arr.forEach(arrItem => {
            if (arrItem.type === "grid") {
              arrItem.columns.forEach(item => {
                this.generateModle(item.dynamicList);
              });
            } else {
              if (
                this.value &&
  ...
  watch: {
    data: {
      deep: true,
      handler(val) {
        this.generateModle(val.dynamicList);
      }
    },
    value: {
      deep: true,
